...
function convertDecimal(num){
  ...
  if(num== null || num == "" || num =="0"){
      ...
      if(num.toString().split(".").length == 2){
          ...
          return num.toString().split(".")[0].replace(/\D/g, "").replace(/\B(?=(\d{3})+(?!\d)\.?)/g, ",") + "."+num.toString().split(".")[1].padStart(2, "0");
      }else{
          // console.log( num.toString().split(".")[0].replace(/\D/g, "").replace(/\B(?=(\d{3})+(?!\d)\.?)/g, ",") + ".00");
          return num.toString().split(".")[0].replace(/\D/g, "").replace(/\B(?=(\d{3})+(?!\d)\.?)/g, ",") + ".00";
      }
  }
                  ...
                  {responsemcdeucli_cliente.etClientesField.map(
                    (response, key) => (
                      ...
                      >
                        ...
                        <th style={{textAlign:'center'}} >{convertDecimal(response.klimkField)}</th>
...
export default Mc_DeuCli_Cliente;
